...
if(defined('TDM_ADD_TO_CART') AND TDM_ADD_TO_CART){
	...
	if(is_array($arCartPrice)){
		...
		if($QUANTITY<intval($arCartPrice['OPTIONS']['MINIMUM'])){$QUANTITY=$arCartPrice['OPTIONS']['MINIMUM'];}
		$arOCBasket = array();
		$arOCBasket['tecdoc'] = "Y";
		...
		//Options
		$arOCBasket['option'][] = Array('name'=>Lng('Article',1,false),'option_value'=>$arCartPrice['ARTICLE'],'value'=>$arCartPrice['ARTICLE'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>Lng('Supplier',1,false),'option_value'=>$arCartPrice['SUPPLIER_STOCK'],'value'=>$arCartPrice['SUPPLIER_STOCK'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>Lng('Dtime_delivery',1,false),'option_value'=>$arCartPrice['DAY'],'value'=>$arCartPrice['DAY'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>Lng('Availability',1,false),'option_value'=>$arCartPrice['AVAILABLE'],'value'=>$arCartPrice['AVAILABLE'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>'Price','option_value'=>$arCartPrice['PRICE'].' '.$arCartPrice['CURRENCY'],'value'=>$arCartPrice['PRICE'].' '.$arCartPrice['CURRENCY'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>'Date','option_value'=>$arCartPrice['DATE_FORMATED'],'value'=>$arCartPrice['DATE_FORMATED'],'type'=>'text');
		$arOCBasket['option'][] = Array('name'=>'Code','option_value'=>$arCartPrice['CODE'],'value'=>$arCartPrice['CODE'],'type'=>'text');
